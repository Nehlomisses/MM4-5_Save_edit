...
def on_member_select(event):
    ...
    """
    try:
        global selected_member  # 聲明使用全局變數 selected_member
        if team_listbox.curselection():  # 檢查是否有選取隊員
            ...
            if selected_member:  # 確保選取了有效的隊員
                member_name = list(team_name_mapping.keys())[list(team_name_mapping.values()).index(selected_member)]  # 獲取隊員名稱
                update_selected_member_label(member_name)  # 更新標籤頁標題
                update_weapon_data(selected_member)  # 更新武器數據
...
def save_to_file():
    ...
    """
    try:
        global selected_member, save_data  # 聲明使用全局變數 selected_member 和 save_data
        if selected_member:  # 檢查是否有選取隊員
            save_weapon_data(selected_member)  # 保存武器數據
            save_attribute_data(selected_member)  # 保存屬性數據
            save_defpon_data(selected_member)  # 保存防具數據
            save_ringpon_data(selected_member)  # 保存配件數據
            save_itempon_data(selected_member)  # 保存雜項數據
...
def load_new_file():
    ...
    從檔案中載入遊戲數據。
    """
    global save_data, game_save_file  # 聲明使用全局變數 save_data 和 game_save_file
